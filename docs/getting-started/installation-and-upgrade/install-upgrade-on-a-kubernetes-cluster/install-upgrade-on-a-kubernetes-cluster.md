...
When the `agent-tls-mode` setting is set to `strict` (which it is by default on new installs of 2.9), you must supply the `--privateCA=true` flag and upload Let's Encrypt's Certificate Authority as outlined in [Adding TLS Secrets](../resources/add-tls-secrets.md). Information on identifying Let's Encrypt's Root CA can be found in the Let's Encrypt [docs](https://letsencrypt.org/certificates/). If this isn't done, then Rancher may fail to connect to new or existing downstream clusters.
