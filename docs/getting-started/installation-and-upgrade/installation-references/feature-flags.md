...
- `uiextension`: Allows UI extensions to load without authentication. The `noAuth` field in the CR must be set to `true` for this feature to work. This flag is enabled by default. Enabling or disabling the flag forces Rancher to restart. The first time this flag is set to `true`, it creates a CRD and enables the controllers and endpoints it needs for the feature to work. If set to `false`, it disables the previously mentioned controllers and endpoints. Setting `uiextension` to `false` has no effect on the CRD -- it will not create a CRD if it does not yet exist, nor will it delete the CRD if it already exists. 
