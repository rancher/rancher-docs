...
| Login Filter | An optional LDAP filter expression that narrows down which users can log in to Rancher based on user LDAP attributes. The User Login Filter must be a valid LDAP filter expression enclosed in parentheses that evaluates to true. For example, to only allow users who are members of a specific group to log in, you could use <code>(memberOf=CN=rancher-users,CN=Users,DC=acme,DC=com)</code>. Note: The User Login Filter is independent of the User Search Filter but it should generally match the search filter results or produce a subset of those results. |
