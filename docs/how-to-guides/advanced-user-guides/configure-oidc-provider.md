...
Rancher can be configured to act as an OpenID Connect (OIDC) provider, enabling other applications to use Rancher for authentication.
It can be used to enable single sign-on (SSO) across Rancher Prime components (TODO link to SUSE OBSERVABILITY DOCS). It adheres to the OIDC specification, allowing it to function like any standard OIDC provider. 
